...
from tensorflow.keras.layers import Input, Dense, Dropout, LSTM, TimeDistributed, Conv1D, BatchNormalization, Concatenate, Attention
...
class DDSR(object):
    def __init__(self, input_dimension, output_dimension, number_of_classes=2, optimizer=None, dual_outputs=False,
        ...
        max_relu_bound=None, adv_penalty=0.01, adv_penalty_l1=0.01, explicitDataDependence=False, usingAdvReg=False,
        useAttention=False, freezeWeights=False, verbose=False):
        ...
        self.buildModel(input_dimension, output_dimension, number_of_classes=number_of_classes, dual_outputs=dual_outputs,
                        ...
                        optimizer=optimizer, reg=reg, freezeWeights=freezeWeights, useAttention=useAttention, verbose=verbose)


    def buildModel(self, input_dimension, output_dimension, number_of_classes=2, optimizer=None, dual_outputs=False,
        loss_threshold=defaultLossThreshold, patience=10, dropout_rate=defaultDropoutRate, useAttention=False,
    ...
    #this method passes inputs multipleForwardPasses times through the DDSR network
    #and averages the results to compute the final output
    #this method returns


    #todo tonight: test this
    def evalMultipleForwardPasses(self, inputData, groundTruth, multipleForwardPasses, batch_size):
        #iteratively gather predictions
        outputArray = np.zeros(shape=(multipleForwardPasses, inputData[0].shape[0], self.output_dimension))
        for curPass in range(multipleForwardPasses):
            outputArray[curPass, :, :] = self.model.predict(inputData, batch_size=batch_size)

        #average across the forward passes collected
        meanOutputs = scipy.stats.gmean(outputArray, axis=0)#np.mean(outputArray, axis=0)#scipy.stats.gmean(outputArray, axis=0)

        #declare final model outputs
        finalOutputs = np.argmax(meanOutputs, axis=1)

        #calculate accuracy
        accuracy = 1/(inputData[0].shape[0])*(np.sum(np.isclose(finalOutputs, np.argmax(groundTruth,axis=1), rtol=1e-1)))
    ...
    def trainOnline(self, inputTrainingData, inputTrainingTargets, inputValidationData, validationTargets, training_epochs=1, #adversarialAttackFunction=create_adversarial_pattern,
                ...
                trainOnAWGNPerts=False, awgnPertsOnly=False, noisyTrainingSigma=1./256., singleAttack=False, multipleForwardPasses=None):
        ...
        history, benignResults, advResults, agnResults, advSFPResults, agnSFPResults= [], [], [], [], [], []
        for curEpoch in range(training_epochs):
            print("current epoch: %s"%str(curEpoch))
            ...
            if ((testBetweenEpochs is not None) and (curEpoch % testBetweenEpochs == 0)):
                if (multipleForwardPasses is not None):
                    benignEval = self.evalMultipleForwardPasses([testingData, testingData], testingTargets, multipleForwardPasses, batch_size=evalBatchSize)
                    benignSFP = self.model.evaluate([testingData, testingData], testingTargets, batch_size=evalBatchSize)[1]
                    SFPAcc, SFPAGNAcc = [benignSFP], [benignEval]
                else:
                    benignEval = self.model.evaluate([testingData, testingData], testingTargets, batch_size=evalBatchSize)[1]
                advAccuracy, noiseAccuracy = [benignEval], [benignEval]
                ...
                for curNoisePower in advPowers:
                    ...
                    #get performances
                    if (multipleForwardPasses is not None):
                        curAcc = self.evalMultipleForwardPasses([testingAttacks, testingAttacks], testingTargets, multipleForwardPasses, batch_size=evalBatchSize)
                        curSFPAcc = self.model.evaluate([testingAttacks, testingAttacks], testingTargets, batch_size=evalBatchSize)[1]
                        SFPAcc.append(curSFPAcc)
                    else:
                        curAcc = self.model.evaluate([testingAttacks, testingAttacks], testingTargets, batch_size=evalBatchSize)[1]
                    advAccuracy.append(curAcc)
                #evaluate agn test acc
                for curNoiseIndex in range(len(noisePowers)):
                    ...
                    if (multipleForwardPasses is not None):
                        agnEval = \
                        self.evalMultipleForwardPasses([corruptedTestX, corruptedTestX], testingTargets, multipleForwardPasses, batch_size=evalBatchSize)
                        agnSFPAcc = self.model.evaluate([corruptedTestX, corruptedTestX], testingTargets, batch_size=evalBatchSize)[1]
                        SFPAGNAcc.append(agnSFPAcc)
                    else:
                        agnEval = self.model.evaluate([corruptedTestX, corruptedTestX], testingTargets, batch_size=evalBatchSize)[1]
                    noiseAccuracy.append(agnEval)
                ...
                if benignResults:
                    ...
                    advResults.append(advAccuracy)
                    if (multipleForwardPasses is not None):
                        advSFPResults.append(SFPAcc)
                        agnSFPResults.append(SFPAGNAcc)
                        benignSFPResults.append(benignSFP)
                    ...
                    advResults = [advAccuracy]
                    if (multipleForwardPasses is not None):
                        advSFPResults = [SFPAcc]
                        agnSFPResults = [SFPAGNAcc]
                        benignSFPResults = [benignSFP]
        ...
        if (multipleForwardPasses):
            return loss, acc, model_path, benignResults, agnResults, advResults, advSFPResults, agnSFPResults, benignSFPResults
        else:
            return loss, acc, model_path, benignResults, agnResults, advResults
        # else:
        #     history = self.model.fit(self.multiInputDataGenerator(trainingData[0], trainingData[1], trainingTargets, keras_batch_size),
...
if __name__ == "__main__":
    ...
    sss = StratifiedShuffleSplit(n_splits=1, test_size=valFrac, random_state=42)
    arrays = indexable(x_train, y_train)
    train, test = next(sss.split(X=x_train, y=y_train))
    iterator = list(chain.from_iterable((safe_indexing(a, train),
                                         safe_indexing(a, test),
                                         train,
                                         test) for a in arrays))
    trainX, valX, train_is, val_is, trainY, valY, _, _ = iterator
    ...
    dataDict = {'trainX':trainX,\
                ...
                'testX':x_test,\
                'testY':y_test}
    ddsrSplitDataFile = open('ddsrSplitDataFile', 'wb')
    pickle.dump(dataDict, ddsrSplitDataFile)
    ddsrSplitDataFile.close()
    ...
    ourModel = DDSR(inputDimension, output_dimension, number_of_classes=numberOfClasses, adv_penalty=0.0025,
                       ...
                       max_relu_bound=1.1, reg='LDR', verbose=True, usingAdvReg=False, stochasticResonatorOn=True)
    ...
    ourModel.model.save_weights('DDSR_resonators_on_256.h5')
